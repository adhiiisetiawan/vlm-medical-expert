...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer("masked_bias", torch.tril(torch.ones(config.block_size, config.block_size))
    ...
    def forward(self, x):
        ...
        att = att.masked_fill(self.masked_bias[:,:,:T,:T] == 0, float('-inf'))
...
class SwitchLayer(nn.Module):
    def __init__(self, config, expert_network:nn.Module, n_expert:int=4, top_k:int=1):
        super().__init__()

        self.switch = nn.Linear(config.n_embd, n_expert)
        self.noise = nn.Linear(config.n_embd, n_expert)
        self.top_k = top_k
...
class MoEBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mlp = SwitchLayer(config, MLP(config), 4, 2) # Swapped MLP to switch layer
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([MoEBlock(config) if i%self.moe_period == self.moe_offset else Block(config) for i in range(config.n_layer)]), # MoE every n layer (starts from 1)
    ...
    def from_pretrained(cls, model_type):
        ...
        print("number of hf parameters: %.2fM" % (n_params/1e6,))

        # Match huggingface/transformers model to the new architecture
        sd_keys = set(sd.keys())
        sd_hf_keys = set(sd_hf.keys())

        broadcasted_keys = list(sd_hf_keys - sd_keys)

        def generate_key_names(original):
            return [original.replace('mlp', f'mlp.experts.{i}') for i in range(4)]

        for old_key in broadcasted_keys:
        ...
        sd_keys = [k for k in sd.keys() if not any(k.endswith(s) for s in ignore_suffixes)] # ignore these
        sd_hf_keys = [k for k in sd_hf.keys() if not any(k.endswith(s) for s in ignore_suffixes)] # ignore these
        transposed = ['c_attn.weight', 'c_proj.weight', 'c_fc.weight']

        # basically the openai checkpoints use a "Conv1D" module, but we only want to use a vanilla nn.Linear.
        # this means that we have to transpose these weights when we import them
        print(f"hf param count: {len(sd_hf_keys)}, loaded param count: {len(sd_keys)}")
    ...
    def get_expert_trigger_count(self):
        count = []
        for block in self.transformer.h[self.moe_offset::self.moe_period]:
            count.append(block.mlp.get_expert_count())
        return count
    
    def reset_expert_trigger_count(self):
        for block in self.transformer.h[self.moe_offset::self.moe_period]:
            block.mlp.reset_expert_count()
